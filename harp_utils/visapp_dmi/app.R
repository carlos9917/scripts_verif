...
# 
# THE APP ASSUMES A FIXED FILENAME CONVENTION OF THE FORM OUTLINED IN fn_png_name (fn_uwcw_plot_helpers.R)
#
...
app_dir <- NULL
app_dir <- "/data/users/cap/R/harp-verif/visualization/visapp_dmi"
if (is.null(app_dir)){
...
if (length(c_exps) > 0){
  ...
  exps_for_tab <- "Error: No valid experiments listed in the directory"
}
...
ui <- shiny::tags$html(
  ...
  shiny::navbarPage( # Add the logo at the top
    title=div(img(src="uwc_logo_blue.png",
                  style="margin-top: -14px;
                  padding-right:10px;
                  padding-bottom:2px;",
                  height = "50px")), 
    windowTitle="Test Shiny Vis",
