...
start_date <-2023010100
end_date <- 2023013123
lead_times_selected <- 1 #seq(0,5,1)
fig_name <- paste0(paste("hour1_freq_dist_mju",substr(start_date,0,8),substr(end_date,0,8),sep="_"),".jpg")
...
print("summary new")
#print(summary(obs_mju_new$TROAD))
obs_mju_new %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_res <- obs_mju_new %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
min_val <- summary(obs_mju_new$TROAD)[1][[1]]
max_val <- summary(obs_mju_new$TROAD)[6][[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)

print("summary old")
#print(summary(obs_mju_short$TROAD))
min_val <- summary(obs_mju_short$TROAD)[1][[1]]
max_val <- summary(obs_mju_short$TROAD)[6][[1]]
sd_res <- obs_mju_short %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)

print("summary all")
#print(summary(obs_mju_long$TROAD))
min_val <- summary(obs_mju_long$TROAD)[1][[1]]
max_val <- summary(obs_mju_long$TROAD)[6][[1]]
sd_res <- obs_mju_long %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)
...
# Create a plot with only tmin and tmax
#ggplot(data_summary, aes(x = valid_date, y = Temperature, color = as.factor(SID))) +
#  geom_line(alpha = 0.5) +  # Add transparency to lines for better visibility
...
#  labs(x = "Date", y = "Temperature", title = "Road Temperature Trends by Station ID") +
#  theme_minimal()
#fig_mm <- paste0(paste("minmax_mju",substr(start_date,0,8),substr(end_date,0,8),sep="_"),".jpg")
#ggsave(fig_mm)


obs_mju_long_summary <- obs_mju_long %>%
  mutate(valid_date = as.Date(valid_dttm)) %>%  # Extract date from datetime
  group_by(valid_date,SID) %>%
  summarize(min_TROAD = min(TROAD), max_TROAD = max(TROAD)) %>%
  summarize(valid_date = valid_date, avg_min_TROAD = mean(min_TROAD), avg_max_TROAD = mean(max_TROAD))

obs_mju_short_summary <- obs_mju_short %>%
  mutate(valid_date = as.Date(valid_dttm)) %>%  # Extract date from datetime
  group_by(valid_date) %>%
  summarize(min_TROAD = min(TROAD), max_TROAD = max(TROAD)) %>%
  summarize(valid_date = valid_date, avg_min_TROAD = mean(min_TROAD), avg_max_TROAD = mean(max_TROAD))
...
#geom_line(data = data_summary, aes(x = valid_date, y = min_TROAD), color = "red")+
  ...
  geom_line(data = obs_mju_long_summary, aes(x = valid_date, y = avg_min_TROAD), color = "blue", linetype = "solid", size = 1) +
  geom_line(data = obs_mju_long_summary, aes(x = valid_date, y = avg_max_TROAD), color = "red", linetype = "solid", size = 1) +
  #geom_line(data = obs_mju_short_summary, aes(x = valid_date, y = avg_min_TROAD), color = "purple", linetype = "dashed", size = 1) +
  #geom_line(data = obs_mju_short_summary, aes(x = valid_date, y = avg_max_TROAD), color = "purple", linetype = "dashed", size = 1) +
  facet_wrap(~ SID, scales = "free_y", ncol = 2) +  # Create a grid of plots, one for each SID
  labs(x = "Date", y = "Temperature", title = "Road Temperature Trends by Station ID") +
  theme_minimal() +
  scale_size_manual(values = c(0.5, 0.5))  # Adjust the thickness of the lines. Not working
