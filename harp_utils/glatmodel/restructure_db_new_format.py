...
#def drop_near_whole_hour(df:pd.DataFrame,df_save:pd.DataFrame) -> pd.DataFrame:
def drop_near_whole_hour(df:pd.DataFrame) -> pd.DataFrame:
    # Round 'valid_dttm' to the nearest hour
    df['date_format'] = pd.to_datetime(df['valid_dttm'], unit='s')
    df['rounded_date'] = df['date_format'].dt.round('H')

    # Calculate the difference between the rounded timestamp and the original timestamp
    df['time_diff'] = df['date_format'] - df['rounded_date']
    #df["time_diff"] = [pd.to_datetime(df['rounded_valid_dttm'].values[k]) - pd.to_datetime(df['date_format'].values[k]) for k,stuff in enumerate(df['date_format'])]
    df["time_diff"] = abs(pd.to_datetime(df['rounded_date']) - pd.to_datetime(df['date_format']))
    df["time_diff"] = df['time_diff'].dt.total_seconds() / 60 #convert to minutes
    #df['time_diff'] = abs(df['valid_dttm'] - df['rounded_valid_dttm'])

    # Sort the DataFrame by 'time_diff' and keep only the rows with the minimum difference for each rounded hour
    #selected_rows = df.sort_values(by='time_diff').groupby('rounded_valid_dttm').head(1)
    # Group by 'SID' and 'rounded_valid_dttm' and find the row with the minimum absolute difference

    df_part1 = df[df['date_format'] < df['rounded_date']]
    df_part2 = df_part1.loc[df_part1.groupby(['SID', 'valid_dttm'])['time_diff'].idxmin()]
    #one more sweep
    df_part2 = df_part2.sort_values(by='time_diff')
    selected_rows = df_part2.drop_duplicates(subset=["SID",'rounded_date'], keep='first')
     
    #this was just for test
    #df_save = df_save[df_save.SID == 338500].sort_values(by=["SID","date_format"])
    #df_save['rounded_date'] = df_save['date_format'].dt.round('H')
    #df_save["time_diff"] = abs(pd.to_datetime(df_save['rounded_date']) - pd.to_datetime(df_save['date_format']))
    #df_save["time_diff"] = df_save['time_diff'].dt.total_seconds() / 60 #convert to minutes
    # Select rows where the time difference is within 15 minutes
    #selected_rows = df[df['time_diff'] <= pd.Timedelta('15 minutes')]
    # Drop the extra columns if needed
    selected_rows["valid_dttm"] =  selected_rows['rounded_date'].astype('datetime64[s]').astype(int)
    selected_rows = selected_rows.drop(columns=['rounded_date', 'time_diff','date_format'])
...
def check_the_dates(df_out:pd.DataFrame) -> None:
    ...
    """
    real_dates=pd.DataFrame({"date":[ts2date(ts) for ts in df_out['valid_dttm']],
                                  "valid_dttm":df_out["valid_dttm"],
                                  "SID":df_out["SID"]})
    #check all stations
    stations = real_dates.drop_duplicates(["SID"])["SID"]
    #df_out.drop_duplicates(['SID', 'valid_dttm'], inplace=True)
    print(f"First dates")
    select_first_date=[]
    ...
    for station in stations:
        ...
        dates["valid_dttm"] = [d - ts_start_date for d in dates["valid_dttm"]]
        dates['lead_time']=dates['valid_dttm'].map(lambda x:ts2date(x).time().hour + ts2date(x).time().minute/60 )
        print(dates.head(10))
...
def restructure_fcst_db(dbase:str, model:str, cfg:dict) -> pd.DataFrame:
    ...
    df.rename(columns={'ID':'SID', 'TIME':'valid_dttm',"FSTEP":"lead_time"}, inplace=True)
    
    for column_name in columns:
        
        df_out = df[['SID', 'valid_dttm','lead_time', column_name]].copy()
        ...
        #TODO:correct the forecast steps (lead_time)
        #check the dates before calculating the lead_time
        #check_the_dates(df_out)
        #sys.exit(0)
        #df_out['lead_time']=df_out['valid_dttm'].map(lambda x:ts2date(x).time().hour + ts2date(x).time().minute/60 )
        #correct strings to add leading zeroes
        #df_out["lead_time"]=[ts.zfill(4) for ts in df_out["lead_time"]]
        #df_out["lead_time"] = [float(ts[0:2]) + float(ts[2:4])/60. for ts in df_out["lead_time"]]

        #drop all the fractional times, still to figure out how to use them
        df_out["lead_time"] = df_out["lead_time"].astype("float")
        df_out = df_out[df_out["lead_time"]%1 != 0.5]
        df_out["lead_time"] = df_out["lead_time"].astype("int64")

        df_out['fcst_dttm']=df_out['valid_dttm']-df_out['lead_time']*3600
        df_out['fcst_dttm']=df_out['fcst_dttm'].astype('int64')
        ...
        if not os.path.isfile(dbase_out):
            schema = f"""CREATE TABLE FC(fcst_dttm INT, lead_time INT, parameter TEXT, SID INT, lat DOUBLE, lon DOUBLE, model_elevation DOUBLE, p DOUBLE, units TEXT, valid_dttm INT, {model}_det DOUBLE);"""
            con=sqlite3.connect(dbase_out)
            print(f"Creating FC table in {dbase_out}")
            cursor = con.cursor()
            cursor.execute(schema)
            cursor = con.cursor()
            schema = """CREATE UNIQUE INDEX index_fcst_dttm_lead_time_SID ON FC(fcst_dttm,lead_time,SID);"""
            cursor.execute(schema)
...
def restructure_obs_db(dbase:str, year:int, cfg:dict) -> None:
    ...
    df_raw.rename(columns={'TIME':'valid_dttm','MEAS': param}, inplace=True)
    df_raw.drop(columns=["ID","SENSOR"],inplace=True)
    df_raw.drop_duplicates(['SID', 'valid_dttm'], inplace=True) 
    dbase_path = os.path.join(cfg['sql_dbs_path'],"OBSTABLE")
    if not os.path.isdir(dbase_path):
        os.makedirs(dbase_path)

    # Drop all instances which are not hourly
    #df_save = df_raw.copy()
    #df_save['date_format'] = pd.to_datetime(df_save['valid_dttm'], unit='s')
    #df_clean = drop_near_whole_hour(df_raw,df_save)
    df_raw = drop_near_whole_hour(df_raw)
    ...
    param_type={"valid_dttm":int, "SID": int, "lat": float, "lon":float, "elev":float,
...
"valid_dttm" INT, "SID" INT, "lat" REAL, "lon" REAL, "elev" REAL,
...
"valid_dttm" INT, "SID" INT, "lat" REAL, "lon" REAL, "elev" REAL,
  "{param}" REAL);"""
         ...
         df_raw_ext.drop_duplicates(['SID', 'valid_dttm'], inplace=True)
         # rearrange the columns
         base_cols=["valid_dttm","SID", "lat","lon","elev"]
         ...
         schema_index = """CREATE UNIQUE INDEX index_valid_dttm_SID ON SYNOP (valid_dttm,SID);"""
         ...
         df_write.drop_duplicates(['SID', 'valid_dttm'], inplace=True)
         # rearrange the columns
         base_cols=["valid_dttm","SID", "lat","lon","elev"]
