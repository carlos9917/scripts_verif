...
hourdat <- fread("merged_r01_glat.csv")
...
# Change  model_ID to quantiles (i.e. identify from lowest to highest for each validity time per initialisation - this identification is necessary to avoid overdispersed errors)
# this changes model_ID to quantiles and expects 10 members to process, all are euk_?? which I do not have No need for this step in my case
#print("before hourdat")
#hourdat[model_type == "glatmodel", model_ID := paste0(round(seq(0,100,100/11), 0),"th%_glatmodel"), by = c("site_ID","validity_time","lead_hrs")]
#print("after hourdat")
...
hourdat <- hourdat[order(site_ID, validity_time, model_type, lead_hrs, forecast_temp)] # change data order as you please, for viewing convenience
...
#length(unique(hourdat[, model_type]))
#
...
ggplot(hourdat[init_time < '2023-01-15 00:00:00' & validity_time > '2023-01-11 00:00:00'][site_ID == unique(site_ID)[1]]) + 
  ...
  geom_line(data = hourdat[validity_time <= '2023-01-15 00:00:00' & validity_time > '2023-01-11 00:00:00'][site_ID == unique(site_ID)[1]],
            aes(x = validity_time, y = obs_temp), col = "black") +
  geom_vline(xintercept = as.numeric(as.POSIXct('2023-01-05', origin = "1970-01-01")), linetype = "dashed") +
  labs(x = "validity time (solid black line represents time zero)", y = "road surface temperature (°C)") +
...
for(i in 1:nscen){
  ...
  site <- sample(unique(hourdat$site_ID), 1) # randomly select a site to use
  #tfs <- unique(hourdat[SSPA_ID == site][order(validity_time)]$validity_time) # extract all possible forecast times
  tfs <- unique(hourdat[site_ID == site][order(validity_time)]$validity_time) # extract all possible forecast times
  ...
  available_tfs <- available_tfs[1:length(available_tfs - max(hourdat[site_ID == site]$lead_hrs))] # exclude forecast times which don't have a long enough future in the data
  ...
  train <- hourdat[site_ID == site & validity_time <= tf & validity_time > (tf - hours(runin*24))] # construct training data as x days runin up to time zero
  ...
  for(m in unique(hourdat$model_type)){
    ...
    test[[m]] <- hourdat[site_ID == site & model_type == m & init_time <= tf & validity_time > tf & validity_time <= (tf + hours(max(hourdat[site_ID == site & model_type == m]$lead_hrs)))]
  }
  test <- rbindlist(test)
...
ggplot(melt(results[, list(postprocessed = sqrt(mean((obs_temp-quant_0.5_mean)^2)), 
                    ...
                    by = c("lead_hrs", "site_ID", "run")][, list(QRF_pp = mean(postprocessed), NWP_avg = mean(avged_nwp)), by = c("lead_hrs")], 
...
for(scen in sample(unique(results$run), nscen)){
  ...
  check_model <- randscen[model_type == "glatmodel"]
  ggplot(check_model) +
  #ggplot(randscen[model_type == "glatmodel"]) +
