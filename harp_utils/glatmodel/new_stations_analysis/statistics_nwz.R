...
fig_name <- paste0(paste("hour1_freq_dist_nwz",substr(start_date,0,8),substr(end_date,0,8),sep="_"),".jpg")
...
print("summary new")
#print(summary(obs_nwz_new$TROAD))
obs_nwz_new %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_res <- obs_nwz_new %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
min_val <- summary(obs_nwz_new$TROAD)[1][[1]]
max_val <- summary(obs_nwz_new$TROAD)[6][[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)

print("summary old")
#print(summary(obs_nwz_short$TROAD))
min_val <- summary(obs_nwz_short$TROAD)[1][[1]]
max_val <- summary(obs_nwz_short$TROAD)[6][[1]]
sd_res <- obs_nwz_short %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)

print("summary all")
#print(summary(obs_nwz_long$TROAD))
min_val <- summary(obs_nwz_long$TROAD)[1][[1]]
max_val <- summary(obs_nwz_long$TROAD)[6][[1]]
sd_res <- obs_nwz_long %>% summarise(sd_var = sd(TROAD,na.rm=TRUE))
sd_val <- sd_res[[1]]
df <- data.frame("min" = min_val, "max" = max_val, "sd" = sd_val)
print(df,row.names=FALSE)
...
#geom_line(data = data_summary, aes(x = valid_date, y = min_TROAD), color = "red")+
  ...
  geom_line(data = obs_nwz_long_summary, aes(x = valid_date, y = avg_min_TROAD), color = "blue", linetype = "solid", size = 1) +
  geom_line(data = obs_nwz_long_summary, aes(x = valid_date, y = avg_max_TROAD), color = "red", linetype = "solid", size = 1) +
  #geom_line(data = obs_nwz_short_summary, aes(x = valid_date, y = avg_min_TROAD), color = "purple", linetype = "dashed", size = 1) +
  #geom_line(data = obs_nwz_short_summary, aes(x = valid_date, y = avg_max_TROAD), color = "purple", linetype = "dashed", size = 1) +
...
fig_mm <- paste0(paste("minmax_nwz",substr(start_date,0,8),substr(end_date,0,8),sep="_"),".jpg")
ggsave(fig_mm)
##############################################################################
...
#ggsave(paste0(paste("hour5_freq_dist_nwz",substr(start_date,0,8),substr(end_date,0,8),sep="_"),".jpg"))
ggsave(fig_name)
